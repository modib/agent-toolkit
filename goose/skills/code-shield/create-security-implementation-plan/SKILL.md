...
is called `run-security-scanner`.
...
Perform a security audit by leveraging your `generate-security-audit-report`
skill.
...
- **Security Audit**: Audit the new code for design-level security issues (input
  ...
  in the `walkthrough.md` artifact using the `generate-security-audit-report`
  skill.
```
...
- All findings should be resolved or explicitly acknowledged before proceeding.
